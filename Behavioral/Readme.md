...
- Iterator: Allows iterating over an object without having to expose the object’s
internal structure (which may change in the future).

- Strategy: Allows passing different algorithms (behaviours) to an object.

- Template Method: Allows defining a template (skeleton) for an operation. Specific steps will then
...
- Mediator: Allows an object to encapsulate the communication between other
objects.

- Chain of Responsibility: Allows building a chain of objects to process a
request.

- Visitor: Allows adding new operations to an object structure without
modifying it.